...
def get_query_tokens(query_string):
    ...
    Applies the same set of lingusitic modules as during
    index construction.
...
def get_doc_to_norm(index, doc_freq, num_docs):
    ...
    """
    # index: {term:[(id, term-freq)] } : {'cat': [(1, 2), (2, 1)], 'door': [(1, 1)], 'water': [(3, 1)]}
    # doc_freq: {term: doc-freq}: {'cat': 2, 'door': 1, 'water': 1}
    # TODO: Edit this function to implement tfidf
    import math

    # calculate the tfidf for each (term,doc-id)
    # get tfidf for each doc as a vector. Each element is for each term.
    # docTfidf = dict()
    # for term, value in index.terms():
    #     docID = value[0]
    #     termFreq = value[1]
    #     docFreq = doc_freq[term]
    #     tfidf = termFreq * math.log((num_docs / 1 + docFreq), math.e)
    #     # if docId is not in the docTfidf
    #     if docID not in docTfidf.keys():
    #         docTfidf[docID] = []
    #
    #     docTfidf[docID].append(tfidf)

    doc_norm = defaultdict(int)
    # calculate square of norm for all docs
    for term in index.keys():
        for doc_id, doc_tf in index[term]:  # value[0] and value[1] of index[term]
            docFreq = doc_freq[term]
            tfidf = doc_tf * math.log((num_docs / (1 + docFreq)), math.e)
            doc_norm[doc_id] += tfidf ** 2

    # calculate the norm
    for doc_id in doc_norm.keys():
        norm = math.sqrt(doc_norm[doc_id])
...
def run_query(query_token_counts, index, doc_freq, doc_norm, num_docs):
    ...
    for query_term, query_tf in query_token_counts:
        # 　get tfidf for each term of query
        if query_term not in doc_freq.keys():
            docFreq=0
        else:
            docFreq = doc_freq[query_term]
        tfidf = query_tf * math.log((num_docs / (1 + docFreq)), math.e)
        query_norm += tfidf ** 2
    ...
    for query_term, query_tf in query_token_counts:
        ...
        for doc_id, doc_tf in index[query_term]:
            ...
            quetfidf = query_tf * math.log((num_docs / (1 + docFreq)), math.e)
            doctfidf = doc_tf * math.log((num_docs / (1 + docFreq)), math.e)


            doc_to_score[doc_id] += quetfidf * doctfidf / (doc_norm[doc_id] * query_norm)
